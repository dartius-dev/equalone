...
void main() {
  group('Equalone.empty', () {
    test('empty values', () {
        ...
        assert(Equalone.empty(Equalone.shallow([])));
    });
  ...
  // Removed duplicate definition of group and test within main
      test('lists equality', () {
        assert(Equalone.shallowEquals([1, 2, 3], [1, 2, 3]));
        assert(Equalone.shallowEquals([1, 2, 3], [3, 2, 1], unordered: true));

        assert(Equalone.shallowEquals(<int>[1, 2, 3], <double>[1.0, 2.0, 3.0]));
        assert(Equalone.shallowEquals(<int>[1, 2, 3], <double>[3.0, 2.0, 1.0], unordered: true));
        
        assert(Equalone.shallowEquals(<num>[1, 2.0, 3], <num>[1.0, 2, 3.0]));
        assert(Equalone.shallowEquals(<num>[1, 2.0, 3], <num>[3.0, 2, 1.0], unordered: true));
      ...
      test('sets equality', () {
        ...
        assert(Equalone.shallowEquals(<int>{1, 2, 3}, <double>{1.0, 2.0, 3.0}));
        assert(Equalone.shallowEquals(<num>{1.0, 2, 3.0}, <num>{1, 2.0, 3}));
        
        assert(Equalone.shallowEquals({1, 2.0, 3}, {1, 2.0, 3}));
        assert(Equalone.shallowEquals({1.0, 2, 3}, {1, 2.0, 3.0}));
      ...
      test('records equality', () {
       ...
       assert(Equalone.shallowEquals((1, 2), (1, 2.0)));
        assert(!Equalone.shallowEquals(({1}, [2]), ({1}, [2])));
      });
      test('null equality', () {
    ...
    group('Equalone.deepEquals', () {
      test('lists equality', () {
        ...
        assert(Equalone.deepEquals([1, 2, 3], [3, 2, 1], unordered: true));
        
        assert(Equalone.deepEquals(<int>[1, 2, 3], <double>[1.0, 2.0, 3.0]));
        assert(Equalone.deepEquals(<int>[1, 2, 3], <double>[3.0, 2.0, 1.0], unordered: true));
        assert(Equalone.deepEquals(<num>[1, 2.0, 3], <num>[1.0, 2, 3.0]));
        assert(Equalone.deepEquals(<num>[1, 2.0, 3], <num>[3.0, 2, 1.0], unordered: true));
